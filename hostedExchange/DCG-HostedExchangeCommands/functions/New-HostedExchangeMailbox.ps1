...
If ($ImportFromCsv -eq $false) {
    ...
    If (($firstName -ne "") -and ($lastName -ne "")) {
        $longName = "$firstName $lastName"; $params = @{
            "Name"        = $longName
    ...
    If (($firstName -ne "") -and ($lastName -eq "")) {
        $params = @{
            "Name"        = $firstName
    ...
    If (($firstName -eq "") -and ($lastName -ne "")) {
        $params = @{
            "Name"        = $lastName
    ...
    sleep 5
    Add-MailboxPermission -Identity $emailAddress -User $adminUser -AccessRight FullAccess -InheritanceType All -Automapping $false
    ...
    Send-MailMessage -From $emailaddress -To $mailto -Subject "Test Message" -Port 2525 -Credential $EmailCredential -SmtpServer $PSEmailServer
...
If ($ImportFromCsv -eq $true) {
    ...
    foreach ($u in $csv) {
        ...
        If ($roleAssignmentPolicy -ne "") {
            ...
            sleep 5
            Add-MailboxPermission -Identity $u.userPrincipalName -User $adminUser -AccessRight FullAccess -InheritanceType All -Automapping $false
            ...
            Send-MailMessage -From $u.userPrincipalName -To $mailto -Subject "Test Message" -Port 2525 -Credential $EmailCredential -SmtpServer $PSEmailServer
        }
    }
}