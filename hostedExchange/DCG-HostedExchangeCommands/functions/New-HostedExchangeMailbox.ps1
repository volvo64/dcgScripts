...
$password = Read-Host "Enter password" -AsSecureString; New-Mailbox -UserPrincipalName $emailAddress -Alias "$firstName.$lastName" -Database $emailDatabase -Name "$firstName $lastName" -OrganizationalUnit $dn  -Password $password -FirstName $firstName -LastName $lastName -DisplayName "$firstName $lastName" -ResetPasswordOnNextLogon $false -AddressBookPolicy $emailABP
Start-Sleep 5
